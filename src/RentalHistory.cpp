...
Rental* RentalHistory::getActiveRental(Customer* customer, const string& title)
{
	for (int i = 0; i < size; i++)
	{
		if (!records[i]->getReturned() && records[i]->getRentBook()->getTitle() == title && records[i]->getBorrower()->getUserCredential().getUserId() == customer->getUserCredential().getUserId())
		{
			return records[i];
		}
	}
...
int RentalHistory::getListSizeByCustomer(Customer*customer) const
{
	int len = 0;
	for (int i = 0; i < size; i++)
	{
		if (records[i]->getBorrower()->getUserCredential().getUserId() == customer->getUserCredential().getUserId())
			len++;
	}
	return len;
}

Rental** RentalHistory::getRentalsByCustomer(Customer* customer)
{
	int len = 0;
	for (int i = 0; i < size; i++)
	{
